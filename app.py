...
@app.route('/')
def index():
  artists = Artist.query.with_entities(Artist.id,Artist.name).order_by(desc(Artist.created_at)).limit(10).all()
  venues = Venue.query.with_entities(Venue.id, Venue.name).order_by(desc(Venue.created_at)).limit(10).all()
